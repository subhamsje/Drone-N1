...
import { LayoutDashboard, Globe2, GitMerge, Box, Search, Wifi } from 'lucide-react';

export const TopCommandBar: React.FC = () => {
  const { workspaceMode, setWorkspaceMode, setCommandPaletteOpen } = useUiStore();
  ...
  return (
    <header className="h-13 w-full bg-[#080c14]/95 border-b border-slate-800/80 px-5 flex items-center justify-between z-30 shrink-0 select-none font-sans backdrop-blur-xl">
        ...
        <nav className="flex items-center space-x-1 bg-slate-900/60 p-1 rounded-xl border border-slate-800/60">
          {navs.map((n) => {
            ...
            return (
              <button
                key={n.id}
                onClick={() => setWorkspaceMode(n.id as WorkspaceMode)}
