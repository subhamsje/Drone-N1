...
import { Plus, Play, CheckCircle2, Cpu, Layers, Globe, Camera, BatteryCharging, Users, ShieldAlert } from 'lucide-react';
...
export function NodeMissionGraph() {
  ...
  const subViewTabs = [
    { id: 'blueprint', label: 'Node Blueprint', icon: Layers },
    { id: 'corridor', label: '3D Corridor', icon: Globe },
    { id: 'survey', label: 'Survey Grid', icon: Camera },
    { id: 'energy', label: 'Energy & PONR', icon: BatteryCharging },
    { id: 'swarm', label: 'Swarm Mesh', icon: Users },
    { id: 'contingency', label: 'Fail-Safe', icon: ShieldAlert },
  ] as const;
  ...
  return (
    <div 
      ...
      className="h-full w-full bg-[#0a0e17] text-slate-200 p-6 flex flex-col font-sans overflow-hidden select-none"
    >
      {/* Header Bar */}
      <div className="flex items-center justify-between border-b border-slate-800/60 pb-4 mb-4">
        <div className="flex items-center space-x-4">
          <span className="px-2.5 py-1 rounded-md bg-slate-800/80 text-slate-300 border border-slate-700/60 text-xs font-mono font-medium">
            STUDIO IDE
          </span>
        ...
        <div className="flex items-center space-x-3">
          {subView === 'blueprint' && (
            <div className="flex items-center gap-1 bg-slate-900/60 border border-slate-800/60 p-1 rounded-lg">
              <button onClick={() => addNode('INSPECT')} className="px-2.5 py-1 rounded bg-slate-800/80 hover:bg-slate-700 text-xs font-medium text-slate-300 flex items-center gap-1">
                <Plus className="w-3.5 h-3.5 text-sky-400" /> Inspect
              </button>
              <button onClick={() => addNode('DETECT')} className="px-2.5 py-1 rounded bg-slate-800/80 hover:bg-slate-700 text-xs font-medium text-slate-300 flex items-center gap-1">
                <Plus className="w-3.5 h-3.5 text-sky-400" /> Detect
              </button>
            </div>
          )}
          ...
          <button 
            ...
            disabled={compiling}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-500 text-xs font-medium text-white shadow-md shadow-sky-600/20 transition-all disabled:opacity-50"
          >
            <Play className="w-3.5 h-3.5" />
            <span>{compiling ? 'Compiling DAG...' : 'Compile MAVSDK'}</span>
          </button>
          
          <button 
            onClick={() => setWorkspaceMode('command_globe')}
            className="px-3.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700/60 text-xs font-medium text-slate-200 transition-all"
          >
            Deploy to 3D &rarr;
          </button>
        </div>
      ...
      {subView === 'contingency' ? (
        ...
        <div className="flex-1 flex gap-4 overflow-hidden">
            ...
            {nodes.map((n) => {
              ...
              return (
                <div
                  ...
                  className={`absolute w-52 p-3.5 rounded-xl border backdrop-blur-md cursor-grab active:cursor-grabbing transition-shadow z-20 ${
                    isSelected
          ...
          <div className="w-80 rounded-xl border border-slate-800/80 bg-slate-900/40 p-4 backdrop-blur-md flex flex-col justify-between">
            <div>
              <h3 className="text-xs font-mono font-semibold uppercase tracking-wider text-slate-300 border-b border-slate-800 pb-2 mb-4">
              ...
              {selectedNode ? (
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="text-xs font-mono font-bold text-sky-400">{selectedNode.id} • {selectedNode.type}</div>
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-white mt-1 focus:border-sky-500/60 focus:outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea 
                      ...
                      className="w-full h-20 bg-slate-950 border border-slate-800 rounded p-2 text-xs font-mono text-slate-200 mt-1 focus:border-sky-500/60 focus:outline-none"
                    />
                  </div>

                  {compiledResult && (
                    <div className="p-3 rounded-lg bg-emerald-950/30 border border-emerald-500/30 text-xs font-mono space-y-1">
            ...
            <div className="border-t border-slate-800 pt-3">
              <button 
                onClick={handleCompileGraph}
                className="w-full py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-mono text-xs font-semibold transition-all flex items-center justify-center gap-2"
              >
                <Cpu className="w-3.5 h-3.5 text-sky-400" /> Compile & Upload to UAV
              </button>
            </div>
          </div>
        </div>
